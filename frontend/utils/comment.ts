...
export async function renderComments(url: string) {
  ...
  if (commentCache) {
    ...
    if (elected_comments.length > 0) {
      ...
      const commentNum = elected_comments.length; // Approximate logic since metadata is gone
      commentHTML += '<div style="max-width: 667px;margin: 0 auto;padding: 10px 10px 80px;">';
      commentHTML += `<p style="font-size: 15px;color: #949494;">留言 ${commentNum}</p>`;
      ...
      for (const comment of elected_comments) {
        ...
        const commentReplyResponse: any = null; // Stubbed
