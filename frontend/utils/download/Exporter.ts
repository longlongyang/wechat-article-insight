...
import { rustBackendFetch } from '~/utils/rustBackendApi';
...
export class Exporter extends BaseDownload {
  ...
  private async downloadResourceTask(url: string, fakeid: string): Promise<void> {
    ...
    */
    // For backend mode, we just skip cache check or assume we always download fresh or use another mechanism.
  ...
  private async exportExcelFiles(): Promise<void> {
    ...
    for (let i = 0; i < total; i++) {
      ...
      const metadata: any = null; // Stubbed
      if (metadata) {
  ...
  private async exportHtmlFiles() {
    ...
    for (let i = 0; i < total; i++) {
      ...
      const resourceMap: any = null; // Stubbed
      if (!resourceMap) {
      ...
      for (const resourceUrl of resourceMap.resources) {
        ...
        const resource: any = null; // Stubbed
        if (!resource) {
          continue;
        }
  ...
  private async exportPdfFiles() {
    ...
    for (let i = 0; i < total; i++) {
      const url = this.urls[i];
      const filename = await this.exportDirName(url);
      console.log(`(${i + 1}/${total})开始导出 PDF: ${filename}(${url})`);

      const content = await this.getPureContent(url, 'html', parser);
      if (!content) {
        console.warn('getPureContent 返回空内容');
        continue;
      }
      ...
      try {
        // 调用 Prince API 生成 PDF
        const response = await rustBackendFetch('/api/pdf', {
          method: 'POST',
          headers: {
  ...
  private async getPureContent(url: string, format: 'html' | 'text', parser: DOMParser): Promise<string> {
    ...
    // 移除 #js_content 的隐藏样式，使正文内容可见
    const $jsContent = $jsArticleContent.querySelector('#js_content') as HTMLElement | null;
    if ($jsContent) {
  ...
  private async normalizeHtml(
    ...
    return $jsArticleContent.outerHTML + commentHTML;
  }
}
