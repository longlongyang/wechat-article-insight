...
async function newLoginSession() {
  ...
  const resp = await rustPost<StartLoginResult>(`/api/web/login/session/${sid}`);
...
async function getQrcode() {
  try {
    ...
    qrcodeSrc.value = `${config.public.rustBackendUrl}/api/web/login/getqrcode?rnd=${Math.random()}`;
    msg.value = '';
...
async function checkQrcodeStatus() {
  const resp = await rustGet<ScanLoginResult>('/api/web/login/scan');
  if (resp && resp.base_resp && resp.base_resp.ret === 0) {
    switch (resp.status) {
      case 0:
        _check();
...
async function bizLogin() {
  try {
    loading.value = true;
    const resp = await rustPost<LoginAccount>('/api/web/login/bizlogin');
    if (resp.err) {
